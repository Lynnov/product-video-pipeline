...
- `KFCV50_BASE_URL` 为空时使用 `https://kfcv50.link`。
...
自动 fallback 只支持同调用格式的 KFC V50 备用 API。切换到备用 API 不需要再次提示用户，因为 `.env` 已显式启用同格式备用通道。
...
- 自动 fallback 只会切到同格式 KFC V50 备用 API，不会切到 `dreamina-cli`。
